...
class ReservationCreateView(ViewWithoutCSFRAuthentication):
    def post(self, request):
        ...
        return JsonResponse({"success":"Reservation created successfully!"}, status = 200)
    ...
    def post(self, request):
        ...
        try:
            ...
            if user.groups.filter(name='NormalUser'):
                return JsonResponse({"msg": "Unallowed Access"}, status = 400)
            else:
                ...
                return JsonResponse({"msg": f"reservation_{reservation.id}'s state is changed successfully."})
        except:
            return JsonResponse({"msg": "Failed to change reservation state"}, status=400)
